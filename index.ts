...
// ==========================================
// ▼▼ 変更点1: 起動時にスラッシュコマンドをDiscordに登録する ▼▼
// ==========================================
client.once('clientReady', async () => {
    console.log(`ログイン完了: ${client.user?.tag} がオンラインになりました！`);

    // Discordに「/search」というコマンドを登録（上書き）する
    try {
        await client.application?.commands.set([
            {
                name: 'bot_search',
                description: 'AIがフォーラム内の過去の論文を検索・推薦します',
                options: [
                    {
                        type: ApplicationCommandOptionType.String,
                        name: 'query',
                        description: '検索したい内容や要望（例: 遠隔操作をやりやすくする技術）',
                        required: true,
                    }
                ]
            }
...
// ==========================================
// ▼▼ 変更点2: スラッシュコマンドが実行された時の処理 ▼▼
// ==========================================
client.on('interactionCreate', async (interaction: Interaction) => {
    // チャットへのスラッシュコマンド入力でなければ無視
    if (!interaction.isChatInputCommand()) return;

    if (interaction.commandName === 'bot_search') {
        const query = interaction.options.getString('query');
        if (!query) return;

        // Discordの仕様で、3秒以内に返事をしないとエラーになるため、
        // 「考え中...」という仮のステータスを先に送信して時間を稼ぐ
        await interaction.deferReply();

        try {
            const forumChannel = await client.channels.fetch(FORUM_CHANNEL_ID);
            if (!forumChannel || !forumChannel.isThreadOnly()) {
                await interaction.editReply("⚠️ エラー: フォーラムチャンネルが見つかりません。");
                return;
            }
            ...
            if (paperList.length === 0) {
                await interaction.editReply("フォーラムにまだ論文が登録されていません。");
                return;
            }
            ...
            if (searchData.matches && searchData.matches.length > 0) {
                ...
                searchData.matches.forEach((match: { id: string; reason: string }) => {
                    const thread = threads.get(match.id);
                    if (thread) {
                        const entry = `**[${thread.name}](<https://discord.com/channels/${interaction.guildId}/${thread.id}>)**\n💡 **推薦理由:** ${match.reason}\n\n`;
                ...
                // 1つ目のメッセージは、deferReplyした「考え中...」を上書きする
                const firstMessage = messageQueue[0];
                if (firstMessage) {
                    await interaction.editReply(firstMessage); 
                }
                
                // 2つ目以降がある場合は、追撃（followUp）で連続投稿する
                for (let i = 1; i < messageQueue.length; i++) {
                    const nextMessage = messageQueue[i];
                    if (nextMessage) {
                ...
                await interaction.editReply(`❌ 「**${query}**」に役立ちそうな論文は、現在のデータベースには見つかりませんでした。`);
            }

        } catch (error) {
            console.error("検索エラー:", error);
            await interaction.editReply("❌ 検索中にエラーが発生しました。ログを確認してください。");
        }
    }
});
// ==========================================
...
client.on('messageCreate', async (message: Message) => {
    ...
    // ※ここにあった \bot や /bot のif文ブロックは不要になったため削除しました
